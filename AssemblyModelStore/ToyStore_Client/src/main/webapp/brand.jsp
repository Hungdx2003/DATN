            ...
            <div class="row">
                ...
                <div class="col-lg-9 col-md-9">
				    <div class="row d-flex justify-content-center">
						...
						<c:set var="brandParams" value="" />
						<c:if test="${not empty paramValues.brands}">
						    <c:set var="brandParams" value="&brands=${fn:join(paramValues.brands, '&brands=')}" />
						</c:if>
				        <div class="col-lg-12 text-center">
						    <div class="pagination__option">
								<c:if test="${currentPage > 1}">
						            <a href="?view=brand${brandParams}&page=${currentPage - 1}"><i class="fa fa-angle-left"></i></a>
						        </c:if>
						        <c:forEach var="page" items="${paginationData}">
						            <c:choose>
						                ...
						                <c:when test="${page == currentPage}">
						                    <a class="active" href="?view=brand${brandParams}&page=${page}">${page}</a>
						                </c:when>
						
						                <c:otherwise>
						                    <a href="?view=brand${brandParams}&page=${page}">${page}</a>
						                </c:otherwise>
						            </c:choose>
						        </c:forEach>
						
						        <c:if test="${currentPage < totalPages}">
						            <a href="?view=brand${brandParams}&page=${currentPage + 1}">
