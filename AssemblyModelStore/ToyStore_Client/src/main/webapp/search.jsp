            ...
            <div class="row">
                <c:choose>
                	<c:when test="${not empty result}">
                		<div class="col-lg-12">
		                	<div class="row">
		                		<h5 style="margin-left: 100px; margin-bottom: 10px;"><i class="bi bi-box-seam"></i> Đã tìm thấy <strong>${totalItems}</strong> kết quả phù hợp</h5>
		                	</div>
						        ...
						        <div class="col-lg-12 text-center">
								    <div class="pagination__option">
										<c:if test="${currentPage > 1}">
								            <a href="?view=search&query=${word}&page=${currentPage - 1}"><i class="fa fa-angle-left"></i></a>
								        </c:if>
								        <c:forEach var="page" items="${paginationData}">
								            <c:choose>
								                ...
								                <c:when test="${page == currentPage}">
								                    <a class="active" href="?view=search&query=${word}&page=${page}">${page}</a>
								                </c:when>
								
								                <c:otherwise>
								                    <a href="?view=search&query=${word}&page=${page}">${page}</a>
								                </c:otherwise>
								            </c:choose>
								        </c:forEach>
								
								        <c:if test="${currentPage < totalPages}">
								            <a href="?view=search&query=${word}&page=${currentPage + 1}">
                	...
                	<c:otherwise>
                		<div class="container mt-5">
				            <div class="text-center">
				                <img src="img/box.png" alt="Giỏ hàng trống" style="width: 300px; max-width: 100%;">
				                <h4 class="mt-4">Không có kết quả nào phù hợp</h4>
				            </div>
				        </div>
                	</c:otherwise>
                </c:choose>
